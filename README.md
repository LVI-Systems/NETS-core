...
# Introduction
Current event contract markets operate on trading architectures that significantly lag behind those of other financial derivatives. This leads to two main issues: **liquidity fragmentation** and **capital inefficiency**:  

- Liquidity in mutually-exclusive event contracts are **siloed into individual order books**, leading to **inefficient execution and value extraction by arbitrage**. Liquidity which should be mathematically linked are fragmented, which means that a fraction of true market liquidity is available to any aggressing order. This results in heightened slippage for large marketable orders, especially in volatile market conditions.
...
This design leads major event contract venues to function primarily as **predatory liquidity harvesting mechanisms**, going against the spirit of **fair price discovery, information aggregation and efficient expression of diverse convictions and strategies**.  
